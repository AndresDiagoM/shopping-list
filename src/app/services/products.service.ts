...
export class ProductsService {
  ...
  getProduct(id: string) {
    return this.http.get<Product>(`${this.ApiFirebase}/${id}.json`);
  }
  ...
  delete(product: Product) {
    return this.http.delete<Product>(`${this.ApiFirebase}/${product.id}${'.json'}`);
  }
  update(id: string, dto: updateProductDTO){
    return this.http.put<Product>(`${this.ApiFirebase}/${id}${'.json'}`, dto);
  }
}
